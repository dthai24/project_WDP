...
async function main() {
  await connectDB();
  const scriptPath = path.join(__dirname, 'migrate-test-config-section-decouple-bank.sql');
  ...
  `);
  const rows = await sql.query('SELECT * FROM dbo.Test_Config_Section ORDER BY ConfigId, SectionOrder');
