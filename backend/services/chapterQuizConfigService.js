...
function findGroupBySectionId(groups, sectionId) {
  const tempId = toSectionTempId(sectionId);
...
function buildSectionRowsFromConfig(config, bankStats) {
  ...
  getSectionQuestionCountsForPart(config, SKILL_VOCABULARY).forEach((entry) => {
    ...
    const bankSectionId = parseSectionIdFromTempId(entry.sectionTempId);
    if (!bankSectionId) {
      throw new Error('Section từ vựng/ngữ pháp không hợp lệ.');
    }

    const group = getInTestGroups(vocabularyGroups).find(
      (item) => parseSectionIdFromTempId(item.sectionTempId) === bankSectionId,
    );
    if (!group) {
      throw new Error(`Section từ vựng/ngữ pháp (SectionId=${bankSectionId}) không còn dùng trong bài kiểm tra.`);
    }
...
function mapBundleToClientConfig(bundle, bankStats = null) {
  ...
  (bundle.configSections ?? []).forEach((row) => {
    ...
    const vocabularyGroups = getSkillGroupsFromBankStats(bankStats, SKILL_VOCABULARY);
    const group = findGroupBySectionId(vocabularyGroups, row.BankSectionId);

    partConfig.sectionQuestionCounts.push({
      sectionTempId: toSectionTempId(row.BankSectionId)
