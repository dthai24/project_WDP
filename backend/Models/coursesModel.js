...
const getCoursePaths = async (courseId) => {
    ...
    const result = await request.query(`Select * From Paths Where CourseId = @courseId ORDER BY [Order] ASC`);
...
const getPathNodes = async (pathId) => {
    ...
    const result = await request.query(`SELECT * FROM [dbo].[Path_Nodes] Where PathId = @pathId Order by NodeOrder Asc`);
    return result.recordset;
}
...
const getMaterials = async (nodeId) => {
    ...
    const result = await request.query(`SELECT * FROM [LearningPath_Base].[dbo].[Node_Materials] Where NodeId = @nodeId Order By MaterialOrder ASC`);
    return result.recordset;
}
