...
export default function MentorRequests() {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/mentor/applications/${selectedApp._id}/review`, {
        method: "POST",
        headers: {
        ...
        <div className="fixed inset-0 bg-slate-950/65 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl overflow-hidden shadow-2xl max-w-3xl w-full border border-slate-100">
            ...
            <div className="p-6 bg-slate-950 flex items-center justify-center max-h-[500px]">
              <img 
                src={`${process.env.REACT_APP_API_URL || "http://localhost:5000"}${showCertificateModal.certificatePath}`} 
                alt="Certificate" 
