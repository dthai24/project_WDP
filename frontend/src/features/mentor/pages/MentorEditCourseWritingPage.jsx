...
import { fetchMentorCourseDetail, updateCourseBasicInfo } from '@/features/mentor/services/mentorCourseService';
import { PRIMARY, TEXT, MUTED } from '@/features/mentor/components/course/mentorCourseCreateStyles';

export default function MentorEditCourseWritingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPrompt(result.course?.FinalWritingPrompt ?? '');
      setLoading(false);
    })();
  ...
  const handleNext = async () => {
    setSaving(true);
    try {
      const result = await updateCourseBasicInfo(courseId, { ...course, FinalWritingPrompt: prompt.trim() });
      if (!result.ok) {
        toast.error(result.message || 'Không thể lưu đề bài Viết.');
        return;
      }
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="mb-4">
        ...
        <p className="text-[14px] mt-1 leading-[1.55] max-w-[620px]" style={{ color: MUTED }}>
          Sau khi vượt qua bài kiểm tra, học viên của <strong>{course.CourseName}</strong> sẽ làm một bài viết cuối khóa được AI chấm điểm. Bạn có thể tự soạn đề bài, hoặc để trống để hệ thống tự sinh đề phù hợp với chủ đề khóa học.
        </p>
      </div>

      <MentorCourseCreateStepIndicator currentStep={4} />

      <Box sx={{ bgcolor: '#fff', border: '1px solid rgba(15,23,42,0.08)', borderRadius: '20px', p: 3, mb: 3 }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 1.25 }}>
        <AppButton
          ...
          onClick={() => navigate(`/mentor/courses/${courseId}/test`)}
          disabled={saving}
