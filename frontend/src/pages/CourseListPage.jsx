...
async function fetchCourses() {
  ...
  return MOCK_COURSES.map((course) => {
    const popularity = course.popularity ?? 600;
    return {
      ...course,
      rating:
        course.rating ??
        Number((4.35 + ((popularity % 550) / 550) * 0.55).toFixed(1)),
      reviewCount: course.reviewCount ?? Math.round(popularity * 0.55),
      studentCount: course.studentCount ?? Math.round(popularity * 24),
    };
  });
}
